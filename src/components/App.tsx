  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      if (
        cfg.bgType === "image" &&
        cfg.bgImage &&
        (cfg.blur > 0 || cfg.brightness !== 100)
      ) {
        const processed = await processImageWithFilters(
          cfg.bgImage,
          cfg.blur,
          cfg.brightness,
        );
    ...
    >
      ...
      <main
        style={{
          ...
          padding:
            "clamp(1rem, 3vw, var(--space-12)) clamp(0.75rem, 4vw, var(--space-16))",
          minHeight: "100vh",
          overflowY: "auto",
        }}
      >
        ...
        >
          ...
          >
            ...
            <div className="cf-status">
              <div
                className={`cf-status-dot ${isProcessing ? "cf-status-dot--processing" : "cf-status-dot--ready"}`}
              />
            </div>
            <button
...
function processImageWithFilters(
  imgSrc: string,
  blur: number,
  brightness: number,
): Promise<string> {
